...
d3.csv('/data/population2021_rangeAges.csv',function (error,data) {
  ...
  var total_byAge = d3.nest()
					   ...
					   .rollup(function(v) { return {
					   	            //"length": v.length,
					   					"F": Math.floor(d3.sum(v,function(d) {return d.F}) / 1000), // / 1000
 ...
 d3.select("#generate").on("click", function() {
	  for (var i=0; i<totals.length; i++) {
	    totals[i].m = Math.random() * dataRange;
	    totals[i].f = Math.random() * dataRange;
	  }
	  refresh(totals);
 ...
 function xChange() {
    ...
    var filteredData = value == "All"? totals : (
															    data.filter(d => {return d.Ethnicity == value   } )
															    								.map(obj => {
																		  							var row = {};
